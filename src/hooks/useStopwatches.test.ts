...
describe('useStopwatches', () => {
  ...
  it('sets the elapsed time of a paused stopwatch outright, leaving lastActiveAt alone', () => {
    // lastActiveAt drives most-recently-used ordering. Writing it here would
    // re-sort the list out from under the cursor mid-edit, so an adjustment
    // must leave it at whatever the last real start/pause/reset set.
